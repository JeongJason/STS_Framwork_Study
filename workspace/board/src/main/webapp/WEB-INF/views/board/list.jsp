					...
					<div style="text-align: center;" class="big-width">
						...
						<c:forEach var="num" begin="${pageDTO.startPage}"
							...
							<c:choose>
								...
								<c:otherwise>
									<a class="chagePage" href="${num}"><code>
											<c:out value="${num}" />
										</code></a>
								</c:otherwise>
							</c:choose>
						...
						<c:if test="${pageDTO.next}">
							<a class="chagePage" href="${pageDTO.endPage +1}"> <code>&gt;</code>
							</a>
						</c:if>
					</div>
					<div style="text-align: center;" class="small-width">
						<c:if test="${pageDTO.criteria.pageNum > 1}">
							<a class="chagePage" href="${pageDTO.criteria.pageNum -1}"> <code>&lt;</code>
							</a>
						</c:if>
						<code>
...
<script>
	...
	$("a.chagePage").on("click", function(e) {
		e.preventDefault();
		// form태그 가져오기
		let $form = $(document.pageForm);
		// form 태그 자식 요소 중 pageNum이 이름인 input태그 가져오기
		// 기존 value값을 사용자가 이동하고자 하는 페이지 번호로 변경하기
